...
export interface Propietario {
  ...
  nombre: string;
  rut: string; // UNIQUE
  email?: string | null; // CH-04, CH-05: debe contener '@' y '.'
...
export interface Consentimiento {
  ...
  pacienteId: number;
  tipo: string; // 'cirugia' | 'eutanasia' | 'hospitalizacion'
  detallesIntervencion: string;
...
export const HCC_SCHEMA_REGISTRY = {
  ...
  relationships: [
    ...
    { from: 'consentimientos.propietario_id', to: 'propietarios.id', type: 'many-to-one' },
    { from: 'consentimientos.paciente_id', to: 'pacientes.id', type: 'many-to-one' }
  ],
};
