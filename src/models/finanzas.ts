...
export interface ComprobantesFiscales {
  ...
  fechaEmision: Date;
  // Auditoría de operador (registra quién emitió el comprobante y su rol)
  operadorId: number;
  operadorRol: string;
}
...
export interface ConveniosSeguros {
  ...
  pacienteId?: number; // FK -> Paciente.id (Mascota específica cubierta)
  coberturaPorcentaje?: number; // Porcentaje global o base cubierto (ej: 80)
  cubreCirugias?: boolean;
  cubreMedicamentos?: boolean;
  medicamentosCobertura?: string; // IDs de medicamentos separados por comas
  cirugiasCobertura?: string; // Nombres/IDs de cirugías cubiertas separadas por comas
}
...
// Nuevas Tablas de Mejoras Financieras L5
export interface ServiciosTarifas {
  id: number;
  nombre: string;
  categoria: string;
  tipo: string;
...
export interface CampanasDescuentos {
  id: number;
  motivo: string;
  porcentaje: number;
  activo: boolean;
...
export interface CompromisosPago {
  id: number;
